...
function _whatsappGuardarGasto(parsed, blob, mime, from, msgId) {
  ...
  // Anexar el remitente al campo NOTAS del pendiente — necesario para
  // que el auto-aprobador (cron) sepa a quién responder vía WhatsApp.
  _whatsappPendienteAnotarFrom(pendId, from);
...
// ════════════════════════════════════════════════════════════════════
//  AUTO-APROBAR — pendientes WhatsApp viejos sin respuesta del usuario
//
//  Si el cliente sube una factura por WhatsApp y no tapea Aprobar ni
//  Cambiar categoría en X minutos, el sistema:
//    1. Aprueba automáticamente el pendiente con la categoría sugerida
//       por la IA al momento del upload.
//    2. Envía confirmación al usuario vía WhatsApp diciendo "auto-
//       aprobado, si querés cambiar abrí la app".
//
//  Configuración
//  ─────────────
//    Script Property WA_AUTO_APROBAR_MINUTOS — minutos de espera
//    antes de auto-aprobar. Default 60.
//
//  Trigger
//  ───────
//    Correr installAutoAprobarWhatsAppTrigger() UNA vez desde el editor
//    para instalar un trigger cada 15 minutos que llame a
//    _whatsappAutoAprobarPendientes(). Cada cliente que use WhatsApp
//    necesita instalarlo en su propio GAS.
// ════════════════════════════════════════════════════════════════════

// Anexa "wa_from:<num>" al campo NOTAS del pendiente recién creado.
function _whatsappPendienteAnotarFrom(pendId, from) {
  try {
    var ss = SpreadsheetApp.openById(CONFIG.SHEET_ID);
    ...
    for (var i = ids.length - 1; i >= 0; i--) {   // buscar desde el fondo (más rápido para el último creado)
      if (String(ids[i][0]) !== pendId) continue;
      var rowNum = i + 3;
      var notasCell = sheet.getRange(rowNum, COL_PEND.NOTAS);
      var notas = String(notasCell.getValue() || '');
      if (notas.indexOf('wa_from:') === -1) {
        notasCell.setValue(notas + ' | wa_from:' + from);
      }
      return;
    }
  } catch(err) { Logger.log('_whatsappPendienteAnotarFrom ERROR: ' + err.message); }
}

// Extrae el wa_from de un string de notas (devuelve '' si no está).
function _whatsappParseFromNotas(notas) {
  var m = String(notas || '').match(/\bwa_from:(\d+)\b/);
  return m ? m[1] : '';
}

// Función principal — invocada por el trigger cada 15 min.
function _whatsappAutoAprobarPendientes() {
  var props = PropertiesService.getScriptProperties();
  var minutosUmbral = parseInt(props.getProperty('WA_AUTO_APROBAR_MINUTOS') || '60', 10);
  var token = props.getProperty('META_WHATSAPP_TOKEN');
  var phoneId = props.getProperty('META_PHONE_ID');
  if (!token || !phoneId) {
    Logger.log('AutoAprobar: faltan META_WHATSAPP_TOKEN o META_PHONE_ID');
    return;
  }

  var ahora = new Date().getTime();
  var umbralMs = minutosUmbral * 60 * 1000;

  var ss = SpreadsheetApp.openById(CONFIG.SHEET_ID);
  var sheet = ss.getSheetByName(SHEET_ACREEDORES_PENDING);
  if (!sheet) { Logger.log('AutoAprobar: sin hoja pending'); return; }
  var lastRow = sheet.getLastRow();
  if (lastRow <= 2) return;

  var data = sheet.getRange(3, 1, lastRow - 2, PEND_NCOLS).getValues();
  var aprobados = 0, errores = 0;

  for (var i = 0; i < data.length; i++) {
    var r = data[i];
    var estado = String(r[COL_PEND.ESTADO - 1] || '').toLowerCase();
    if (estado !== 'borrador') continue;

    var notas = String(r[COL_PEND.NOTAS - 1] || '');
    var waFrom = _whatsappParseFromNotas(notas);
    if (!waFrom) continue;   // solo pendientes que vinieron por WhatsApp

    var fechaRegStr = r[COL_PEND.FECHA_REG - 1];
    if (fechaRegStr instanceof Date) fechaRegStr = Utilities.formatDate(fechaRegStr, 'America/Panama', 'yyyy-MM-dd HH:mm:ss');
    var fechaRegMs = new Date(String(fechaRegStr).replace(' ', 'T')).getTime();
    if (isNaN(fechaRegMs)) continue;

    var edadMs = ahora - fechaRegMs;
    if (edadMs < umbralMs) continue;   // todavía dentro del plazo

    var pendId = String(r[COL_PEND.ID - 1] || '');
    var acreedor = String(r[COL_PEND.ACREEDOR_NOM - 1] || 'Sin proveedor');
    var total = parseFloat(r[COL_PEND.TOTAL - 1] || 0);
    var catSug = String(r[COL_PEND.CATEGORIA - 1] || 'otros_deducibles');

    Logger.log('AutoAprobar: ' + pendId + ' (edad ' + Math.round(edadMs/60000) + 'min, umbral ' + minutosUmbral + 'min)');

    try {
      var aprRes = _handleAprobarAcreedor({ id: pendId });
      var aprJson = JSON.parse(aprRes.getContent());
      if (!aprJson.success) { errores++; Logger.log('AutoAprobar fail ' + pendId + ': ' + aprJson.error); continue; }
      aprobados++;

      // Confirmar al usuario vía WhatsApp
      _whatsappReply(waFrom,
        '⏱ Auto-aprobado tras ' + minutosUmbral + ' min sin respuesta.\n\n' +
        '📦 ' + acreedor + '\n' +
        '💵 B/. ' + total.toFixed(2) + '\n' +
        '📋 Categoría: ' + _waCatLabel(catSug) + ' (sugerida por IA)\n' +
        '✅ Registrado: ' + (aprJson.egreso_id || pendId) + '\n\n' +
        'Si querés cambiar la categoría:\n' + _whatsappFrontendUrl() + '#registroGastos',
        token, phoneId);
    } catch(err) {
      errores++;
      Logger.log('AutoAprobar EXCEPTION ' + pendId + ': ' + err.message);
    }
  }

  if (aprobados || errores) {
    Logger.log('═══ AutoAprobar resumen: ' + aprobados + ' aprobado(s), ' + errores + ' error(es) ═══');
  }
}

// Instalar el trigger periódico — correr UNA vez desde el editor por cliente.
function installAutoAprobarWhatsAppTrigger() {
  // Limpiar triggers existentes con el mismo handler para evitar duplicados
  var triggers = ScriptApp.getProjectTriggers();
  for (var i = 0; i < triggers.length; i++) {
    if (triggers[i].getHandlerFunction() === '_whatsappAutoAprobarPendientes') {
      ScriptApp.deleteTrigger(triggers[i]);
      Logger.log('Trigger anterior eliminado');
    }
  }
  ScriptApp.newTrigger('_whatsappAutoAprobarPendientes')
    .timeBased()
    .everyMinutes(15)
    .create();
  Logger.log('✅ Trigger instalado — corre cada 15 min');
}

// Desinstalar el trigger (por si querés desactivar la feature).
function removeAutoAprobarWhatsAppTrigger() {
  var triggers = ScriptApp.getProjectTriggers();
  var removed = 0;
  for (var i = 0; i < triggers.length; i++) {
    if (triggers[i].getHandlerFunction() === '_whatsappAutoAprobarPendientes') {
  ...
  Logger.log(removed ? ('✅ ' + removed + ' trigger(s) eliminado(s)') : '⚠️ Ningún trigger instalado');
}
