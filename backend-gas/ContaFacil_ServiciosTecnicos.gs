...
var SHEET_INV_MST   = 'Inventario_Maestro';   // columna 10 = exist_actual
var SHEET_INV_MOV   = 'Inventario_Movimientos';
var COL_INV_CODIGO  = 1;   // A  codigo_sku
var COL_INV_EXIST   = 10;  // J  exist_actual
var COL_MOV_ID      = 1;   // A  id_movimiento
var COL_MOV_FECHA   = 2;   // B  fecha
var COL_MOV_TIPO    = 3;   // C  tipo (entrada|salida)
var COL_MOV_SKU     = 4;   // D  codigo_sku
var COL_MOV_DESC    = 5;   // E  descripcion_sku
var COL_MOV_QTY     = 6;   // F  cantidad
var COL_MOV_COSTO   = 7;   // G  costo_unitario
var COL_MOV_TOTAL   = 8;   // H  costo_total
var COL_MOV_ORIGEN  = 9;   // I  origen
var COL_MOV_NFAC    = 10;  // J  num_factura
var COL_MOV_ID_ST   = 11;  // K  id_st
var COL_MOV_CLI     = 12;  // L  cliente_proveedor
var COL_MOV_DRIVE   = 13;  // M  drive_url
var COL_MOV_NOTAS   = 14;  // N  notas
var MOV_NCOLS       = 14;
...
// ═══════════════════════════════════════════════════════════════
//  _revertirInventarioST  — v1.4
//  Crea entradas compensatorias en Inventario_Movimientos y
//  actualiza exist_actual en Inventario_Maestro para revertir
//  las salidas que generó este ST.
//  Solo actúa sobre movimientos con tipo='salida' e id_st=idST.
//  Retorna el número de SKUs revertidos.
// ═══════════════════════════════════════════════════════════════
function _revertirInventarioST(ss, idST, motivo) {
  var revertidos = 0;
  var stamp      = Utilities.formatDate(new Date(), 'America/Panama', 'yyyy-MM-dd HH:mm');

  var sheetMov = ss.getSheetByName(SHEET_INV_MOV);
  var sheetMst = ss.getSheetByName(SHEET_INV_MST);
  if (!sheetMov || !sheetMst) {
    Logger.log('⚠️  _revertirInventarioST: hojas de inventario no encontradas');
    return 0;
  }

  var lastMov = sheetMov.getLastRow();
  if (lastMov < 2) return 0;

  // Leer todos los movimientos (fila 1 = cabecera doble, datos desde fila 2)
  var dataMov = sheetMov.getRange(2, 1, lastMov - 1, MOV_NCOLS).getValues();

  // Leer Inventario_Maestro (fila 1 = cabecera, datos desde fila 2)
  var lastMst  = sheetMst.getLastRow();
  var dataMst  = sheetMst.getRange(2, 1, lastMst - 1, COL_INV_EXIST).getValues();

  // Construir índice SKU → fila (base 1 desde la hoja)
  var skuIndex = {};
  for (var m = 0; m < dataMst.length; m++) {
    var sku = String(dataMst[m][COL_INV_CODIGO - 1] || '').trim();
    if (sku) skuIndex[sku] = m + 2;  // fila real en la hoja
  }

  // Nuevo correlativo para los movimientos de reversión
  var maxMov = 0;
  for (var x = 0; x < dataMov.length; x++) {
    var mid = String(dataMov[x][COL_MOV_ID - 1] || '');
    var parts = mid.split('-');
    var n = parseInt(parts[parts.length - 1], 10);
    if (!isNaN(n) && n > maxMov) maxMov = n;
  }

  // Procesar salidas del ST
  var toAdd = [];
  for (var i = 0; i < dataMov.length; i++) {
    var idST_mov = String(dataMov[i][COL_MOV_ID_ST - 1] || '').trim();
    var tipo     = String(dataMov[i][COL_MOV_TIPO   - 1] || '').trim().toLowerCase();
    if (idST_mov !== idST || tipo !== 'salida') continue;

    var sku      = String(dataMov[i][COL_MOV_SKU   - 1] || '').trim();
    var desc     = String(dataMov[i][COL_MOV_DESC  - 1] || '').trim();
    var qty      = parseFloat(dataMov[i][COL_MOV_QTY  - 1] || 0) || 0;
    var costoU   = parseFloat(dataMov[i][COL_MOV_COSTO - 1] || 0) || 0;
    var costoT   = parseFloat(dataMov[i][COL_MOV_TOTAL - 1] || 0) || 0;

    if (!sku || qty <= 0) continue;

    maxMov++;
    var newId = 'MOV-CEYCO-' + new Date().getFullYear() + '-' + String(maxMov).padStart(4, '0');

    toAdd.push([
      newId,
      stamp,
      'entrada',     // reversión de salida
      sku,
      desc,
      qty,
      costoU,
      ...
      '',            // cliente_proveedor
      '',            // drive_url
      'Reversión por ' + (motivo || 'cancelación') + ' de ST ' + idST
    ]);

    // Actualizar exist_actual en Inventario_Maestro
    if (skuIndex[sku]) {
      var mstRow  = skuIndex[sku];
      var existAct = parseFloat(sheetMst.getRange(mstRow, COL_INV_EXIST).getValue() || 0) || 0;
      sheetMst.getRange(mstRow, COL_INV_EXIST).setValue(existAct + qty);
      Logger.log('✅ Inventario revertido: ' + sku + ' +' + qty + ' → ' + (existAct + qty));
    }

    revertidos++;
  }

  // Añadir filas de reversión al final de Inventario_Movimientos
  if (toAdd.length > 0) {
    sheetMov.getRange(sheetMov.getLastRow() + 1, 1, toAdd.length, MOV_NCOLS).setValues(toAdd);
    SpreadsheetApp.flush();
    Logger.log('✅ ' + toAdd.length + ' movimiento(s) de reversión añadidos para ST: ' + idST);
  }
...
function _handleCancelarST(params, callback) {
  ...
  try {
    ...
    // ── 4. Revertir movimientos de inventario si estaba en ejecución ──────
    if (estadoActual === 'en_ejecucion') {
      var invRevertidos = _revertirInventarioST(ss, idST, motivo);
      if (invRevertidos > 0) {
        Logger.log('✅ Inventario revertido: ' + invRevertidos + ' SKU(s) para ST ' + idST);
      }
    }
...
function _handleEliminarST(params, callback) {
  ...
  try {
    ...
    // ── 3. Borrar ingreso por ID exacto ──────────────────────────────────
    ...
    // ── 4. Revertir movimientos de inventario (antes de borrar ST_Items) ──
    var invRevertidos = _revertirInventarioST(ss, idST, 'eliminación ST');
    if (invRevertidos > 0) {
      Logger.log('✅ Inventario revertido: ' + invRevertidos + ' SKU(s) para ST ' + idST);
    }

    // ── 5. Borrar filas ST_Items (descendente para no desplazar índices) ──
